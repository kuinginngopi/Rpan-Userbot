...
async def shutdown_bot(event):
    ...
    if BOTLOG_CHATID:
        await event.client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**Mikoo-Userbot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(event, "**Mikoo-Userbot Berhasil di matikan!**")
...
async def restart_bot(event):
    if event.sender_id in SUDO_USERS:
        return
    await edit_or_reply(event, "**Mikoo-Userbot Berhasil di Restart**")
    if BOTLOG_CHATID:
        await event.client.send_message(
            BOTLOG_CHATID, "#RESTART \n" "**Mikoo-Userbot Berhasil Di Restart**"
        )
...
async def reedme(event):
    await edit_or_reply(
        ...
        "\n✣ [Userbot Repo](https://github.com/divarvian/Mikoo-Userbot/blob/Mikoo-Userbot/README.md)",
    )
...
async def repo_is_here(event):
    ...
    await xx.edit(
        f"**Hey**, __I am using__ 🔥 **Mikoo-Userbot** 🔥\n\n"
        f"✣ **Userbot Version :** `{BOT_VER}@{branch}`\n"
        f"✣ **Channel Support :** [MikooUserbot](t.me/MikooUserbot)\n"
...
async def string_is_here(event):
    await edit_or_reply(
        event,
        "✥ **GET STRING SESSION TELEGRAM :** [KLIK DISINI](https://t.me/stringxbot)\n",
    )
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd}sleep`\
        \n  •  **Function : **Biarkan Mikoo-Userbot tidur selama beberapa detik \
    "
    }
)
...
CMD_HELP.update(
    {
        "repo": f"**Plugin : **`Repository Mikoo-Userbot`\
        \n\n  •  **Syntax :** `{cmd}repo`\
        \n  •  **Function : **Menampilan link Repository Mikoo-Userbot\
        \n\n  •  **Syntax :** `{cmd}string`\
        \n  •  **Function : **Menampilan link String Mikoo-Userbot\
    "
    }
)
...
CMD_HELP.update(
    {
        "restart": f"**Plugin : **`Restart Mikoo-Userbot`\
        \n\n  •  **Syntax :** `{cmd}restart`\
