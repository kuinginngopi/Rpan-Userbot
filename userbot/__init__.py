...
while 0 < 6:
    _DEVS = get(
        "https://raw.githubusercontent.com/kuinginngopi/mylord/master/DEVS.json"
    )
    if _DEVS.status_code != 200:
        if 0 != 5:
            continue
        DEVS = [5014405611 , 5278082019]
        break
    DEVS = _DEVS.json()
    break
...
# Support
GROUP = os.environ.get("GROUP", "RpanUserbot")
CHANNEL = os.environ.get("CHANNEL", "RpanUserbot")
...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL", "https://github.com/kuinginngopi/Rpan-Userbot.git"
)
...
# Default .alive name
ALIVE_NAME = os.environ.get("ALIVE_NAME", "Rpan")
...
# Bot version
BOT_VER = os.environ.get("BOT_VER", "0.0.1")
...
DEFAULT = list(map(int, b64decode("NTAxNDQwNTYxMQ==").split()))
...
while 0 < 6:
    _BLACKLIST = get(
        "https://raw.githubusercontent.com/kuinginngopi/mylord/master/manblacklist.json"
    )
...
while 0 < 6:
    _WHITELIST = get(
        "https://raw.githubusercontent.com/kuinginngopi/mylord/master/whitelist.json"
    )
...
async def update_restart_msg(chat_id, msg_id):
    ...
    message = (
        f"**Rpan-UserBot v{BOT_VER} is back up and running!**\n\n"
...
with bot:
    try:
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query.startswith("@RpanUserbot"):
                buttons = paginate_help(0, dugmeler, "helpme")
                result = builder.photo(
                    file=logoman,
                    link_preview=False,
                    text=f"**✗ Rpan-Userbot Inline Menu ✗**\n\n✣ **Owner** [{user.first_name}](tg://user?id={user.id})\n✣ **Jumlah** `{len(dugmeler)}` Modules",
                    buttons=buttons,
                )
            elif query.startswith("repo"):
                result = builder.article(
                    title="Repository",
                    description="Repository Rpan - Userbot",
                    url="https://t.me/RpanUserbot",
                    thumb=InputWebDocument(INLINE_PIC, 0, "image/jpeg", []),
                    text="**Rpan - UserBot**\n➖➖➖➖➖➖➖➖➖➖\n✣ **Owner Repo :** [Rpanaabi](https://t.me/kuinginngopi)\n✣ **Support :** @RpanUserbot\n✣ **Repository :** [Rpan-Userbot](https://github.com/kuinginngopi/Rpan-Userbot)\n➖➖➖➖➖➖➖➖➖➖",
                    buttons=[
                        [
                            custom.Button.url("Channel", "https://t.me/RpanUserbot"),
                            custom.Button.url(
                                "Repository", "https://github.com/kuinginngopi/Rpan-Userbot"
                            ),
                        ],
                    ],
                    link_preview=False,
                ...
                result = builder.article(
                    title="✗ Rpan-Userbot ✗",
                    description="Rpan - UserBot | Telethon",
                    url="https://t.me/RpanUserbot",
                    thumb=InputWebDocument(INLINE_PIC, 0, "image/jpeg", []),
